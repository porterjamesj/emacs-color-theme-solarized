...
This is my (James Porter's) fork of this package, which isn't
maintained anymore and has been removed from MELPA, but which I like
because it's the only solarized package for emacs that supports
terminals well.

The primary change I made is ripping out support for the obsolete
color-theme mode. I also merged a few PRs updating fonts for various
packages that had been sitting open for a long time on the original
repo.
